...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className="flex h-screen bg-gray-900 text-gray-100">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-gray-950 border-r border-gray-800 flex flex-col transform transition-transform duration-200 ease-in-out ${
      ...
      >
        ...
        <div className="px-4 py-4 border-t border-gray-800">
          <div className="bg-gradient-to-r from-indigo-600/20 to-purple-600/20 rounded-lg p-3 border border-indigo-500/20">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-semibold text-indigo-400">PRO 플랜</span>
              <span className="text-[10px] bg-indigo-500/30 text-indigo-300 px-1.5 py-0.5 rounded">활성</span>
            </div>
            <p className="text-[11px] text-gray-400">계정 5개 / 무제한 자동화</p>
            <div className="mt-2 w-full bg-gray-700 rounded-full h-1.5">
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 h-1.5 rounded-full" style={{ width: "60%" }} />
            </div>
            <p className="text-[10px] text-gray-500 mt-1">3/5 계정 사용 중</p>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top bar */}
        <header className="bg-gray-950/80 backdrop-blur-sm border-b border-gray-800 px-4 lg:px-6 py-3 flex items-center justify-between">
          <button
